...
crosslet.MapView = (function(_super) {
  ...
  MapView.prototype.initialize = function(el, config) {
    ...
    L.tileLayer("http://{s}.tiles.mapbox.com/v3/landplanner.map-khn9uycz/{z}/{x}/{y}.png", this.config.map.leaflet).addTo(this.map);
    // Bill plays with fire
    var topPane = this.map._createPane('leaflet-top-pane', this.map.getPanes().mapPane);
    var topLayer = new L.tileLayer('https://{s}.tiles.mapbox.com/v3/landplanner.map-7o5ro71l/{z}/{x}/{y}.png', {
      maxZoom: 18
    }).addTo(this.map);
